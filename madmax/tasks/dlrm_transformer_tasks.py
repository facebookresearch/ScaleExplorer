...
class DLRM_Transformer_Task(Task):
    ...
    def build_fwd(self, model, system):
        ...
        for transformer_layer_num in range(model.num_transformer_layers):
            ...
            if transformer_layer_num == 0:
                attn_deps = {'EMB_f_c', 'MLPBot{}_f.'.format(mlp_bot_layer_num), 'MLPBot{}_f_c_act_ar.'.format(mlp_bot_layer_num), 'Attn{}_f_c_wgt_ag.'.format(transformer_layer_num)}
            else:
        ...
        for mlp_top_layer_num in range(model.num_top_mlp_layers):
            ...
            t_mlp = self.local_layer_flops / self.eff_flops
            if mlp_top_layer_num == 0:
                mlp_deps = {'TransformerFC{}_f.'.format(transformer_layer_num), 'TransformerFC{}_f_c_act_ar.'.format(transformer_layer_num), 'MLPTop{}_f_c_wgt_ag.'.format(mlp_top_layer_num)}
            else:
    ...
    def build_bwd(self, model, system, t_comp_start, t_comm_start, freeze_top_mlp, freeze_bot_mlp, freeze_emb, freeze_transformer):
        computation_stream = []
        communication_stream = []
        t_comp = t_comp_start
        t_comm = t_comm_start

        assert not freeze_top_mlp or not freeze_bot_mlp or not freeze_emb or not freeze_transformer, 'need to have at least one component that needs to be trained.'
        ...
        if not freeze_bot_mlp or not freeze_emb or not freeze_transformer:
            for transformer_layer_num in range(model.num_transformer_layers-1, -1, -1):
                ...
                if transformer_layer_num == model.num_transformer_layers-1:
                    fc_deps = {'MLPTop{}_b.'.format(mlp_top_layer_num), 'MLPTop{}_b_c_act_ar.'.format(mlp_top_layer_num), 'TransformerFC{}_b_c_wgt_ag.'.format(transformer_layer_num)}
                else:
                ...
                attn_deps = {'TransformerFC{}_b.'.format(transformer_layer_num+1), 'TransformerFC{}_b_c_act_ar.'.format(transformer_layer_num+1), 'Attn{}_b_c_wgt_ag.'.format(transformer_layer_num)}
        ...
        if not freeze_bot_mlp:
            ...
            for mlp_bot_layer_num in range(model.num_bot_mlp_layers-1, -1, -1):
                ...
                if self.mlp_parallel[0] == 'fsdp':
                    ...
                    t_comp, t_comm = self.add_trace('MLPBot{}_b_c_wgt_ag.'.format(mlp_bot_layer_num), t_mlp_c, mlp_deps, 'comm', computation_stream, communication_stream, t_comp, t_comm)
                    self.t_allgather_total += t_mlp_c
                # Handle computation
                t_mlp = 2 * self.local_layer_flops / self.eff_flops
                if mlp_bot_layer_num == model.num_bot_mlp_layers-1:
    ...
    def build_pretrain(self, model, system):
        ...
        b_computation_stream, b_communication_stream, b_t_comp, b_t_comm = self.build_bwd(model, system, t_end, t_end, freeze_top_mlp=False, freeze_bot_mlp=False, freeze_emb=False, freeze_transformer=False)
    ...
    def build_finetune(self, model, system):
        ...
        b_computation_stream, b_communication_stream, b_t_comp, b_t_comm = self.build_bwd(model, system, t_end, t_end, self.freeze_top, self.freeze_bot, self.freeze_emb, self.freeze_transformer)
        t_end = max(b_t_comp, b_t_comm)
    ...
    def check_parallelization_strats(self, model, num_devices, num_intra_node_devices, restrict2d):
        ...
        if restrict2d:
            assert len(self.mlp_parallel) < 3 and len(self.emb_parallel) < 3 and len(self.transformer_parallel) < 3 , "Only allow for up to 2D parallelism for now"
    ...
    def get_parallelization_factors(self):
        ...
        return mlp_duplicate_factor, mlp_shard_factor, emb_duplicate_factor, emb_shard_factor, transformer_duplicate_factor, transformer_shard_factor
    ...
    def get_task_flops(self, model):
        ...
        local_total_flops = local_layer_flops * model.num_mlp_layers + (local_attention_layer_flops + local_transformer_fc_layer_flops) * model.num_transformer_layers
        global_total_flops = global_layer_flops * model.num_mlp_layers + (global_attention_layer_flops + global_transformer_fc_layer_flops) * model.num_transformer_layers

        return local_layer_flops, global_layer_flops, local_attention_layer_flops, global_attention_layer_flops, \
    ...
    def print_summary_stats(self):
        total_cap_per_device = self.mlp_cap_per_device + self.emb_cap_per_device + self.transformer_cap_per_device

        print('**************************************************')
        super().print_summary_stats()
        if self.type == 'finetune':
            print('Frozen Components:')
            print('\tTop MLP: {}, Bot MLP: {}, EMB: {}, Transformer: {}'.format(self.freeze_top, self.freeze_bot, self.freeze_emb, self.freeze_transformer))
        print('Task Memory Usage:')
        print('\tModel Weights: {:.2f} ({:.2f} MLP, {:.2f} EMB, {:.2f} Transformer) GB per device.'.format(total_cap_per_device/1e9, self.mlp_cap_per_device/1e9, self.emb_cap_per_device/1e9, self.transformer_cap_per_device/1e9))
        print('Task FLOPs:')
    ...
    def __init__(
        ...
        self.mlp_cap_per_device, self.emb_cap_per_device, self.transformer_cap_per_device = self.get_mem_usage(model)
        self.local_layer_flops, self.global_layer_flops, self.local_attention_layer_flops, self.global_attention_layer_flops, \
