...
class DLRM_MoE_Task(Task):
    ...
    def build_fwd(self, model, system):
        ...
        for mlp_top_layer_num in range(model.num_top_mlp_layers):
            ...
            if self.mlp_parallel[0] == 'fsdp':
                # FSDP allgather time proportional to output buffer size.
                mlp_layer_bytes = (model.num_experts * model.mlp_layer_params/self.mlp_shard_factor * self.mlp_parallel_degree[0]) * model.bytes_per_nonemb_param 
                t_mlp_c = mlp_layer_bytes / system.eff_allgather_bw[-len(self.mlp_parallel)] # FSDP is always first level of parallelism.
            ...
            for parallel_lvl, (strat, deg) in enumerate(zip(self.mlp_parallel, self.mlp_parallel_degree)):
                if strat == 'tp': # only need to all2all if experts are split across devices
                    activations_bytes = self.local_mlp_bs * model.mlp_dim * model.bytes_per_nonemb_param / model.num_experts
                    t_activations_c = activations_bytes / system.eff_all2all_bw[-len(self.mlp_parallel)+parallel_lvl]
                    t_comp, t_comm = self.add_trace('MoE{}_f_c_act_all2all1.'.format(mlp_top_layer_num), t_activations_c, {'MoE{}_f_c_act_all2all2.'.format(mlp_top_layer_num-1)}, 'comm', computation_stream, communication_stream, t_comp, t_comm)
                    self.t_all2all_total += t_activations_c
            # Handle computation
            t_mlp = self.local_top_layer_flops / self.eff_flops
            if mlp_top_layer_num == 0:
                mlp_deps = {'EMB_f_c', 'MLPBot{}_f.'.format(mlp_bot_layer_num), 'MLPBot{}_f_c_act_ar.'.format(mlp_bot_layer_num), 'MLPTop{}_f_c_wgt_ag.'.format(mlp_top_layer_num), 'MoE{}_f_c_act_all2all1.'.format(mlp_top_layer_num)}
            else:
                mlp_deps = {'MLPTop{}_f.'.format(mlp_top_layer_num-1), 'MLPTop{}_f_c_act_ar.'.format(mlp_top_layer_num-1), 'MLPTop{}_f_c_act_MoE.'.format(mlp_top_layer_num-1), 'MLPTop{}_f_c_wgt_ag.'.format(mlp_top_layer_num), 'MoE{}_f_c_act_all2all1.'.format(mlp_top_layer_num)}
            ...
            for parallel_lvl, (strat, deg) in enumerate(zip(self.mlp_parallel, self.mlp_parallel_degree)):
                if strat == 'ddp':
                    ...
                    activations_bytes = self.local_mlp_bs * model.mlp_dim * model.bytes_per_nonemb_param / model.num_experts
                    t_activations_c = activations_bytes / system.eff_all2all_bw[-len(self.mlp_parallel)+parallel_lvl]
                    t_comp, t_comm = self.add_trace('MoE{}_f_c_act_all2all2.'.format(mlp_top_layer_num), t_activations_c, {'MLPTop{}_f.'.format(mlp_top_layer_num), 'MLPTop{}_f_c_act_ar.'.format(mlp_top_layer_num)}, 'comm', computation_stream, communication_stream, t_comp, t_comm)
                    self.t_all2all_total += t_activations_c
    ...
    def build_bwd(self, model, system, t_comp_start, t_comm_start, freeze_top_mlp, freeze_bot_mlp, freeze_emb):
        ...
        for mlp_top_layer_num in range(model.num_top_mlp_layers-1, -1, -1):
            ...
            for parallel_lvl, (strat, deg) in enumerate(zip(self.mlp_parallel, self.mlp_parallel_degree)):
                if strat == 'tp': # only need to all2all if experts are split across devices
                    activations_bytes = self.local_mlp_bs * model.mlp_dim * model.bytes_per_nonemb_param / model.num_experts
                    t_activations_c = activations_bytes / system.eff_all2all_bw[-len(self.mlp_parallel)+parallel_lvl]
                    t_comp, t_comm = self.add_trace('MoE{}_b_c_act_all2all1.'.format(mlp_top_layer_num), t_activations_c, {'MoE{}_b_c_act_all2all2.'.format(mlp_top_layer_num+1)}, 'comm', computation_stream, communication_stream, t_comp, t_comm)
                    self.t_all2all_total += t_activations_c
            # Handle computation
            t_mlp = 2 * self.local_top_layer_flops / self.eff_flops
            if mlp_top_layer_num == model.num_top_mlp_layers-1:
                mlp_deps = {'MLPTop{}_b_c_wgt_ag.'.format(mlp_top_layer_num), 'MoE{}_b_c_act_all2all1.'.format(mlp_top_layer_num)}
            else:
                mlp_deps = {'MLPTop{}_b.'.format(mlp_top_layer_num+1), 'MLPTop{}_b_c_act_ar.'.format(mlp_top_layer_num+1), 'MLPTop{}_b_c_wgt_ag.'.format(mlp_top_layer_num), 'MoE{}_b_c_act_all2all1.'.format(mlp_top_layer_num)}
            ...
            for parallel_lvl, (strat, deg) in enumerate(zip(self.mlp_parallel, self.mlp_parallel_degree)):
                if strat == 'ddp':
                    if not freeze_top_mlp:
                        mlp_layer_bytes = (model.num_experts * model.mlp_layer_params/shard_factor) * model.bytes_per_nonemb_param
                    ...
                    activations_bytes = self.local_mlp_bs * model.mlp_dim * model.bytes_per_nonemb_param / model.num_experts
                    t_activations_c = activations_bytes / system.eff_all2all_bw[-len(self.mlp_parallel)+parallel_lvl]
                    t_comp, t_comm = self.add_trace('MoE{}_b_c_act_all2all2.'.format(mlp_top_layer_num), t_activations_c, {'MLPTop{}_b.'.format(mlp_top_layer_num), 'MLPTop{}_b_c_act_ar.'.format(mlp_top_layer_num)}, 'comm', computation_stream, communication_stream, t_comp, t_comm)
                    self.t_all2all_total += t_activations_c
        ...
        if not freeze_bot_mlp:
            ...
            for mlp_bot_layer_num in range(model.num_bot_mlp_layers-1, -1, -1):
                ...
                if mlp_bot_layer_num == model.num_bot_mlp_layers-1:
                    mlp_deps = {'MLPTop{}_b.'.format(mlp_top_layer_num), 'MLPTop{}_b_c_act_ar.'.format(mlp_top_layer_num), 'MoE{}_b_c_act_all2all2.'.format(mlp_top_layer_num), 'MLPBot{}_b_c_wgt_ag.'.format(mlp_bot_layer_num)}
                else:
    ...
    def get_task_flops(self, model):
        ...
        return local_bot_layer_flops, global_bot_layer_flops, local_top_layer_flops, global_top_layer_flops, local_total_flops, global_total_flops
    ...
    def print_summary_stats(self):
        ...
        print('Task FLOPs:')
        print('\tBot MLP Layer FLOPs per local batch: {:.2f} TFLOPs.'.format(self.local_bot_layer_flops/1e12))
        print('\tBot MLP Layer FLOPs per global batch: {:.2f} TFLOPs.'.format(self.global_bot_layer_flops/1e12))
        print('\tTop MLP Layer FLOPs per local batch: {:.2f} TFLOPs.'.format(self.local_top_layer_flops/1e12))
        print('\tTop MLP Layer FLOPs per global batch: {:.2f} TFLOPs.'.format(self.global_top_layer_flops/1e12))
    ...
    def __init__(
        ...
        self.local_bot_layer_flops, self.global_bot_layer_flops, self.local_top_layer_flops, self.global_top_layer_flops, \
            self.local_total_flops, self.global_total_flops = self.get_task_flops(model)
