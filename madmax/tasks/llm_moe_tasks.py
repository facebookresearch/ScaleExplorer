    ...
    def build_fwd(self, model, system):
        ...
        for transformer_layer_num in range(model.num_transformer_layers):
            ...
            if transformer_layer_num == 0:
                ...
                attn_deps = {'TransformerFC{}_f.'.format(transformer_layer_num-1), 'TransformerFC{}_f_c_act_ar.'.format(transformer_layer_num-1), 'MoE{}_f_c_act_all2all2.'.format(transformer_layer_num-1), 'Attn{}_f_c_wgt_ag.'.format(transformer_layer_num)}
            ...
            if self.transformer_parallel[0] == 'fsdp':
                # FSDP allgather time proportional to output buffer size.
                fc_layer_bytes = (model.num_experts * model.transformer_fc_layer_params/self.transformer_shard_factor * self.transformer_parallel_degree[0]) * model.bytes_per_nonemb_param 
            ...
            for parallel_lvl, (strat, deg) in enumerate(zip(self.transformer_parallel, self.transformer_parallel_degree)):
                if strat == 'tp': # only need to all2all if experts are split across devices
                    activations_bytes = self.local_transformer_bs * model.transformer_seq_len * model.attention_dim * model.bytes_per_nonemb_param / model.num_experts
                    t_activations_c = activations_bytes / system.eff_all2all_bw[-len(self.transformer_parallel)+parallel_lvl]
                    t_comp, t_comm = self.add_trace('MoE{}_f_c_act_all2all1.'.format(transformer_layer_num), t_activations_c, {'Attn{}_f_c_act_ar.'.format(transformer_layer_num)}, 'comm', computation_stream, communication_stream, t_comp, t_comm)
                    self.t_all2all_total += t_activations_c
            # Handle computation
            t_fc = self.local_transformer_fc_layer_flops / self.eff_flops
            fc_deps = {'Attn{}_f.'.format(transformer_layer_num), 'Attn{}_f_c_act_ar.'.format(transformer_layer_num), 'TransformerFC{}_f_c_wgt_ag.'.format(transformer_layer_num), 'MoE{}_f_c_act_all2all1.'.format(transformer_layer_num)}
            ...
            for parallel_lvl, (strat, deg) in enumerate(zip(self.transformer_parallel, self.transformer_parallel_degree)):
                if strat == 'ddp':
                    ...
                    activations_bytes = self.local_transformer_bs * model.transformer_seq_len * model.attention_dim * model.bytes_per_nonemb_param / model.num_experts
                    t_activations_c = activations_bytes / system.eff_all2all_bw[-len(self.transformer_parallel)+parallel_lvl]
                    t_comp, t_comm = self.add_trace('MoE{}_f_c_act_all2all2.'.format(transformer_layer_num), t_activations_c, {'TransformerFC{}_f.'.format(transformer_layer_num), 'TransformerFC{}_f_c_act_ar.'.format(transformer_layer_num)}, 'comm', computation_stream, communication_stream, t_comp, t_comm)
                    self.t_all2all_total += t_activations_c
        ...
        for transformer_layer_num in range(model.num_transformer_layers-1, -1, -1):
            ...
            for parallel_lvl, (strat, deg) in enumerate(zip(self.transformer_parallel, self.transformer_parallel_degree)):
                if strat == 'tp': # only need to all2all if experts are split across devices
                    activations_bytes = self.local_transformer_bs * model.transformer_seq_len * model.attention_dim * model.bytes_per_nonemb_param / model.num_experts
                    t_activations_c = activations_bytes / system.eff_all2all_bw[-len(self.transformer_parallel)+parallel_lvl]
                    t_comp, t_comm = self.add_trace('MoE{}_b_c_act_all2all1.'.format(transformer_layer_num), t_activations_c, {'Attn{}_b_c_act_ar.'.format(transformer_layer_num+1)}, 'comm', computation_stream, communication_stream, t_comp, t_comm)
                    self.t_all2all_total += t_activations_c
            # Handle computation
            t_fc = 2 * self.local_transformer_fc_layer_flops / self.eff_flops
            fc_deps = {'Attn{}_b.'.format(transformer_layer_num+1), 'Attn{}_b_c_act_ar.'.format(transformer_layer_num+1), 'TransformerFC{}_b_c_wgt_ag.'.format(transformer_layer_num), 'MoE{}_b_c_act_all2all1.'.format(transformer_layer_num)}
            ...
            for parallel_lvl, (strat, deg) in enumerate(zip(self.transformer_parallel, self.transformer_parallel_degree)):
                if strat == 'ddp':
                    if not freeze_transformer and transformer_layer_num < min_frozen_layer:
                        fc_layer_bytes = (model.num_experts * model.transformer_fc_layer_params/shard_factor) * model.bytes_per_nonemb_param
                    ...
                    activations_bytes = self.local_transformer_bs * model.transformer_seq_len * model.attention_dim * model.bytes_per_nonemb_param / model.num_experts
                    t_activations_c = activations_bytes / system.eff_all2all_bw[-len(self.transformer_parallel)+parallel_lvl]
                    t_comp, t_comm = self.add_trace('MoE{}_b_c_act_all2all2.'.format(transformer_layer_num), t_activations_c, {'TransformerFC{}_b.'.format(transformer_layer_num), 'TransformerFC{}_b_c_act_ar.'.format(transformer_layer_num)}, 'comm', computation_stream, communication_stream, t_comp, t_comm)
                    self.t_all2all_total += t_activations_c
            ...
            attn_deps = {'TransformerFC{}_b.'.format(transformer_layer_num), 'TransformerFC{}_b_c_act_ar.'.format(transformer_layer_num), 'MoE{}_b_c_act_all2all2.'.format(transformer_layer_num), 'Attn{}_b_c_wgt_ag.'.format(transformer_layer_num)}
